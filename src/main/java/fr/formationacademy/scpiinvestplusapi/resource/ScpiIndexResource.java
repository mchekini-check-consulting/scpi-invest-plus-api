...
public class ScpiIndexResource {
        ...
        public ResponseEntity<?> searchScpi(
                        @Parameter(description = "Nom de la SCPI (recherche approximative)", example = "Comète") @RequestParam(required = false) String name,
                        @Parameter(description = "Taux de distribution minimum", example = "10") @RequestParam(required = false) BigDecimal distributionRate,
                        @Parameter(description = "Montant minimum de souscription", example = "100") @RequestParam(required = false) Integer minimumSubscription,
                        @Parameter(description = "Frais de souscription (true = avec frais, false = sans frais)") @RequestParam(required = false) Boolean subscriptionFees,
                        @Parameter(description = "Fréquence de paiement (Mensuelle ou Annuel)", example = "Mensuelle") @RequestParam(required = false) String frequencyPayment,
                        @Parameter(description = "Liste des locations pour la recherche", example = "[\"France\", \"Italie\"]") @RequestParam(required = false) List<String> locations,
                        @Parameter(description = "Liste des secteurs pour la recherche", example = "[\"Santé\", \"Bureaux\"]") @RequestParam(required = false) List<String> sectors) {
                try {
                        ...
                        if (results.isEmpty()) {
                                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                                                .body(Map.of(
                                                                "status", "error",
                                                                "message",
                                                                "Aucune SCPI ne correspond aux critères de recherche"));
                        }
        ...
        @Operation(summary = "Récupérer les SCPI avec scheduledPayment à true", description = "Retourne toutes les SCPI de l'index Elasticsearch dont le champ 'scheduledPayment' est à true")
        @ApiResponses({
                ...
                try {
                        ...
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                        .body(Map.of(
                                                        "status", "error",
                                                        "message",
                                                        "Erreur lors de la récupération des SCPI avec scheduledPayment à true",
                                                        "details", e.getMessage()));
                }
        }

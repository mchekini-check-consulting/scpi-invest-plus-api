...
class ScpiServiceTest {
    ...
    private Scpi scpiEntity;
    private ScpiDTO expectedScpiDTO;
    ...
    void getScpiDetailsById_ShouldReturnScpiDTO_WhenScpiExists() {
        ...
        ScpiDTO expectedScpiDTO = ScpiDTO.builder()
                .id(1)
                .name("InvestPlus")
                .minimumSubscription(1000)
                .manager("SCPI Gestion")
                ...
                .advertising("High return, diversified portfolio.")
                .build();
        log.info("Excepted Scpi result : {}", expectedScpiDTO);
        when(scpiRepository.findById(1)).thenReturn(Optional.of(scpiEntity));
        when(scpiMapper.toDTO(scpiEntity)).thenReturn(expectedScpiDTO);

        ScpiDTO result = underTest.getScpiDetailsById(1);
    ...
    void getScpiDetailsById_ShouldReturnNull_WhenScpiDoesNotExist() {
        ...
        // Act
        ScpiDTO result = underTest.getScpiDetailsById(2);
        log.info("Result: {}", result);
        // Assert
        assertNull(result);
        ...
        verify(scpiMapper, never()).toDTO(any());
    }
}
